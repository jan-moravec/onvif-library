...
LIBRARY_DIR=onvif-library-device
...
$WSDL2H -O4 -g -c++11 -P -t $OUTPUT_DIR/typemap.dat -o $OUTPUT_DIR/onvif.h $WSDL_URLS

# patch onvif.h
LINE_NUM="$(grep -n "wsa5.h" $OUTPUT_DIR/onvif.h | head -n 1 | cut -d: -f1)"
sed -i $LINE_NUM'i#import "wsse.h"' $OUTPUT_DIR/onvif.h

# generate bindings
$SOAPCPP2 -I $GSOAP_DIR/import -d $OUTPUT_DIR -c++11 -S -j -L -x $OUTPUT_DIR/onvif.h
